...
resource "aws_s3_bucket" "s3_static_bucket" {
  bucket = "${var.project}-${var.environment}-s3-bucket-${random_string.s3_unique_key.result}"
...
data "aws_iam_policy_document" "s3_static_bucket" {
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = [aws_cloudfront_origin_access_identity.cloudfront_s3_origin_access_identity.iam_arn]
    }
  }
}
